...
def main(debug: bool) -> None:
    ...
    for log in Logs:
        ...
        log_path = Path(script_dir) / log.filename
        ...
        with subprocess.Popen(["gnuplot"], stdin=subprocess.PIPE, encoding='utf8') as gnuplot:
            ...
            gnuplot.stdin.write(f'set label "{clean_filename}" at graph 0.03, 0.9 front\n')
            gnuplot.stdin.write(f"set xdata time\n")
            gnuplot.stdin.write(f"set timefmt \"%Y-%m-%dT%H-%M\"\n")

            gnuplot.stdin.write("set xtics out rotate by -45\n")
